      ...
      blocks: [
        {
          ...
          content:
            "До появления виртуальных окружений жизнь Python-разработчика была адом. Проект A требует `Django 1.5`, проект B — `Django 3.0`, и они не уживаются в одной системе. Сюжет: ты ставишь зависимости одного проекта, ломаешь второй, чинишь второй — ломаешь первый. Сегодня `venv` — обыденность, но **в Node.js аналог появился раньше**, и Python-сообщество долго это переваривало.",
        },
        {
          type: "funfact",
          emoji: "📦",
          title: "left-pad: 11 строк, которые сломали интернет",
          content:
            "В марте 2016 разработчик удалил npm-пакет `left-pad` (11 строк кода для добавления нулей слева). Сломались **тысячи проектов** включая Babel, React. Все полагались на этот крошечный пакет через transitive deps. С тех пор npm запретил удалять пакеты, а индустрия начала всерьёз думать про supply chain security.",
        },
        {
          type: "funfact",
          emoji: "🐛",
          title: "Первый «баг» был настоящим жуком",
          content:
            "В 1947 году Грейс Хоппер записала в журнал Mark II «**First actual case of bug being found**» — нашли мотылька, застрявшего в реле компьютера. Они приклеили его в лог скотчем. Сам термин «bug» использовался ещё Эдисоном в 1878, но именно этот мотылёк увековечил выражение «debug».",
        },
      ],
    },
  ],
