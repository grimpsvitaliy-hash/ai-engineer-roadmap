      ...
      blocks: [
        {
          ...
          content:
            "В RFC 2324 от 1 апреля 1998 года определён статус-код **418 I'm a teapot** — «я чайник, не могу заварить кофе». Это шутка, но многие реальные сервисы (Google.com/teapot) её поддерживают. В 2017 разработчики Node.js пытались убрать его из стандартной библиотеки — **поднялся такой хайп**, что код вернули обратно. Чайник победил.",
        },
        {
          type: "funfact",
          emoji: "🕊️",
          title: "Самые секси домены",
          content:
            "**Domain `example.com`** зарезервирован для документации с 1992 года. Никто не может его купить — он создан специально для примеров в RFC. Аналогично `localhost` (127.0.0.1) — выделенный диапазон. До 2008 года был популярный домен `microsoft.com/eolas` потому что Microsoft проиграла Eolas патентный иск на $565 миллионов из-за **HTML тега `<embed>`**.",
        },
        {
          type: "funfact",
          emoji: "🔐",
          title: "Случайно слитый API-ключ за 5 минут",
          content:
            "Каждые **15 секунд** на GitHub публикуют новый секретный ключ. Боты сканируют коммиты в реальном времени. Реальная история: разработчик закоммитил AWS-ключ → получил счёт **$80 000 за 24 часа** (кто-то майнил криптовалюту на его аккаунте). Поэтому **.env в .gitignore** — это не педантизм, это финансовая защита.",
        },
      ],
    },
  ],
