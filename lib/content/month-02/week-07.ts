      ...
      blocks: [
        {
          type: "funfact",
          emoji: "🧠",
          title: "У LLM нет краткосрочной памяти",
          content:
            "Когда ты говоришь Claude «запомни моё имя», он не запоминает. **Каждый запрос — новый Claude**, который читает всю историю заново. Это как актёр, который каждое утро забывает кто он, и весь день читает дневник предыдущего себя. Метафора жуткая, но правдивая. Поэтому реальные продукты на LLM строят «память» отдельно — БД с фактами о пользователе, RAG по истории общения.",
        },
        {
          type: "funfact",
          emoji: "📐",
          title: "JSON моложе чем кажется",
          content:
            "**JSON был придуман в 2001 году** Дугласом Крокфордом. До этого все мучились с XML. Крокфорд: «Я не изобрёл JSON. Я только обнаружил его в JavaScript — он там уже был». Сейчас JSON — *lingua franca* интернета, и без него LLM API не существовали бы. По иронии, **JSON Schema** (тип-система для JSON) — это XML по сложности.",
        },
        {
          type: "funfact",
          emoji: "🎯",
          title: "Structured output решает 80% багов",
          content:
            "Опытные инженеры скажут: **90% производственных проблем с LLM — из-за парсинга свободного текста**. Модель отвечает «Sure, here's the JSON: ```json {...} ```», твой код ждёт чистый JSON — падает. Anthropic в 2024 году внедрили **JSON mode** именно из-за тысяч жалоб разработчиков. Если ты с самого начала используешь structured outputs — ты обходишь главные грабли.",
        },
      ],
    },
  ],
